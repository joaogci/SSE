...
void init_sse_state(uint64_t seed) {
    ...
    vtx_type = create_vtx_type_list(hberg_system.J, hberg_system.delta, hberg_system.h, hberg_system.C);
...
void loop_update() {
    ...
    while (true) {
        ...
        for (le = 0; le < 4; le++) {
            if (next_double() <= vtx_type[sse_state.vtx[p] - 1].prob_exit[li][le]) {
                new_vtx[p] = vtx_type[sse_state.vtx[p] - 1].new_vtx_type[li][le];
                break;
            }
        }
        j_out = 4 * p + le;
    ...
    for (p = 0; p < sse_state.M; p++) {
        b = (sse_state.op_string[p] / 2) - 1;
        sse_state.op_string[p] = 2 * b + vtx_type[new_vtx[p] - 1].type;
    }
    ...
    for (i = 0; i < hberg_system.N; i++) {
        if (sse_state.first[i] != -1) {
            ...
            hberg_system.spin[i] = vtx_type[new_vtx[p] - 1].spin[l];
        }
        else {
            if (next_double() <= 0.5) {
