...
for i, filename in enumerate(filenames):
    with open("some_results/" + filename, "r") as file:
        ...
        for j in range(T_vals):
            T[i, j], E[i, j], C[i, j], m[i, j], m2[i, j], m_s[i, j], m2_s[i, j], n[i, j] = [float(x) for x in file.readline().strip().split(",")]
        T[i, :] = 1.0 / T[i, :]
...
plt.xlabel(r"$T/J$", fontsize=16)
plt.ylabel(r"$\langle m2 \rangle$", fontsize=16)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.legend(fontsize=16)
...
plt.xlabel(r"$T/J$", fontsize=16)
plt.ylabel(r"$\langle m2 \rangle_s$", fontsize=16)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.legend(fontsize=16)
